...
void app_main(void)
{
    ...
    g_cal[0] = (float)tg_cal[0] * GRES / (float)CAL_ITER;
    g_cal[1] = (float)tg_cal[1] * GRES / (float)CAL_ITER;
    g_cal[2] = (float)tg_cal[2] * GRES / (float)CAL_ITER;
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(cycle_timer, PER)); //5000 us
    ...
    while(1) {
        if (*run) {
            ...
            dt = 0.01; //(float)(cur_time - prev_time) / 1000000.0; // PWM Timer Init causing large inital value
            ...
            gx_ang += ((float)g[0] * GRES - g_cal[0]) * dt; //- g_cal[0]
            ...
            *run = 0;
            ...
            ESP_LOGI(TAG, "%f %f  G: %f %f %f\n\r", x_filt, y_filt, gx_ang, gy_ang, dt
            );
